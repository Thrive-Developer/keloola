...
export async function router(
  ...
  switch (operation) {
    case operations.getAll.value: {
      url = `${ENV.ACCOUNTING_BASE_URL}/sale-quote`;
      method = 'GET';
    ...
    case operations.get.value:
      url = `${ENV.ACCOUNTING_BASE_URL}/sale-quote/${executeFunctions.getNodeParameter('id', 0)}`;
      method = 'GET';
      break;

    case operations.delete.value:
      url = `${ENV.ACCOUNTING_BASE_URL}/sale-quote/delete/${executeFunctions.getNodeParameter('id', 0)}`;
      method = 'DELETE';
      break;

    case operations.send.value: {
      url = `${ENV.ACCOUNTING_BASE_URL}/sale-quote/send/${executeFunctions.getNodeParameter('id', 0)}`;
      method = 'POST';
    ...
    case operations.print.value:
      url = `${ENV.ACCOUNTING_BASE_URL}/sale-quote/print/${executeFunctions.getNodeParameter('id', 0)}`;
      method = 'POST';
      body = {
    ...
    case operations.update.value: {
      ...
      url =
        operation === operations.create.value
          ? `${ENV.ACCOUNTING_BASE_URL}/sale-quote`
          : `${ENV.ACCOUNTING_BASE_URL}/sale-quote/${id}`;
